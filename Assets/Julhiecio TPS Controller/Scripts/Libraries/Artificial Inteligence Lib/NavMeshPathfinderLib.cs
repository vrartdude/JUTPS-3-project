...
namespace JUTPS.AI
{
    public class JUPathFinder
    {
        ...
        public static Vector3[] CalculatePath(Vector3 SourcePosition, Vector3 TargetPosition)
        {
            ...
            NavMeshHit hitNv;
            if (!NavMesh.SamplePosition(SourcePosition, out hitNv, 2, NavMesh.AllAreas))
            {
                Debug.LogWarning("Unable to calculate path, make sure you baked the NavMesh");
            ...
            {
                NavMeshHit hit;
                NavMesh.SamplePosition(SourcePosition, out hit, 2, NavMesh.AllAreas);
            ...
            if (navmesh_path.status == NavMeshPathStatus.PathInvalid)
            {
                ...
                NavMeshHit hitEnd;
                NavMesh.SamplePosition(TargetPosition, out hitEnd, 2, NavMesh.AllAreas);
